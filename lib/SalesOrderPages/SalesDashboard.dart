...
class _SalesDashboardState extends State<SalesDashboard> {
  ...
  Widget appBarTitle = new Text("Search Party", style: TextStyle(color: Colors.purple));
  Icon actionIcon = new Icon(Icons.search);
  TextEditingController controller = new TextEditingController();
  bool loading = true;
  final formatter = new DateFormat("yyyy-MM-dd");
  final String primaryOrdersurl = Constant.ip_url+ 'Sap/getPrimaryOrders';

  Future<Null> getPrimaryOrder() async {
    setState(() => loading = true);
    final response = await http.get(Uri.parse(primaryOrdersurl));
    var responseJson = json.decode(response.body) as List;
    // var tag = responseJson.map((response)=>PrimaryOrder.fromJson(response)).toList();
    setState(() {
      //for (Map party in responseJson) {
        // _primaryOrder.add(PrimaryOrder.fromJson(party));
        _primaryOrder= responseJson.map((response)=>PrimaryOrder.fromJson(response)).toList();
        loading = false;
        // }
    }
   );

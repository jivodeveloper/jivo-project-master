...
late PrimaryOrder orderListItem;
...
class _DetailsState extends State<Details> {
  final String orderItemUrl = Constant.ip_url + "Sap/getPrimaryOrderItems";

  bool loading = true;
  final formatter = new DateFormat("yyyy-MM-dd");
  List<CreateOrderRequest> create = [];

  CreateOrderRequest createOrderRequest = new CreateOrderRequest();
  final String deleteOrderUrl = Constant.ip_url + "Sap/deletePrimaryOrder";
  ...
  Future<Null> getPrimaryOrderItems(String docEntry) async {
    ...
    final response = await http.get(Uri.parse(getoutUrl2));
    //var responseJson = OrderItem.fromJson(json.decode(response.body));
    var responseJson = json.decode(response.body);
    // var tag = responseJson.map((response)=>OrderItem.fromJson(response)).toList();
    setState(() {
      orderItem =responseJson.map((response)=>OrderItem.fromJson(response)).toList();
      loading = false;
    });
  }

  Future deletePrimary(int index, PrimaryOrder createOrderRequest) async {
    String deleteUrl = deleteOrderUrl + '?DocEntry=' + createOrderRequest.docEntry.toString();

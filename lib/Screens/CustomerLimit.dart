...
class _SearchCustLimitState extends State<SearchCustLimit> {
  ...
  Future<Null> getParties() async {
    ...
    final response = await http.get(Uri.parse(getparties));
    final responseJson = json.decode(response.body) as List;
    // var tag = responseJson.map((response)=>PartyModel.fromJson(response)).toList();
    setState(() {
    //  for (Map party in responseJson) {
        _party= responseJson.map((response)=>PartyModel.fromJson(response)).toList();
        loading = false;
    //  }
    });
  }
  ...
  final String getparties = Constant.ip_url+ "Web/getParties";

}

const double _kPickerSheetHeight = 216.0;

late PartyDetailmodel partyDetail;
String gCardCode="";
...
class _CustomerLimitState extends State<CustomerLimit> {
  final String getoutUrl =
      Constant.ip_url + 'Web/getCustomerOutstanding';
  ...
  static final getPostUrl = Constant.ip_url +'Web/createLimit';

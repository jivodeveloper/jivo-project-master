...
class _RealiseReportState extends State<RealiseReport> {
  ...
  final String url = Constant.ip_url + 'SapSales/getSales';

  List<Sales> glbSalesList =[];
  ...
  _asynMethod() async {
    ...
    if (fromdate == "" && todate == "") {
      ...
      fromdate = DateFormat.yMd().format(fromDate);
      todate = DateFormat.yMd().format(toDate).toString();

      setState(() {
        variety = (varietyResult.length == 0 ? null : varietyResult.toString()) as String;
      ...
      onPressed: () async {
        ...
        if (_formKey.currentState!.validate()) {
          if (fromdate == "" && todate == "") {
            ...
            setState(() {
              variety =
                  (varietyResult.length == 0 ? null : varietyResult.toString()) as String;
              type = selectedFilterType;
            });
            Map<String, dynamic> body = new Map();
            body.putIfAbsent("fromdate", () => fromdate);
  ...
  Future<List<http.Response>?> getSalesReport(salesReport) async {
    ...
    if (response.statusCode == 200) {
      ...
      print("resonse.data${response.body}");

      final parsed = json.decode(response.body.toString()).cast<Map<String, dynamic>>();
      await Future.value();
      setState(() {
        List<Sales> salesList = parsed.map<Sales>((json) => Sales.fromJson(json)).toList();
        ...
        salesDbHelper.totalLtrs().then((String liters) {
          ...
          // setState(() => sumLitres = double.parse(liters.toString()));
          glbSalesList.removeWhere((sales) => sales != null);
          glbSalesList.add(sales);
        });
      }
                        ...
                        Flexible(
                          child: InkWell(
                            ...
                            child: Container(
                                ...
                                width: 150,
                                child: new Text(
                                 // formatter.format(glbSalesList[index].liters),
                ...
                Card(
                  ...
                  child: Container(
                    child: MultiSelectFormField(
                      ...
                      hintWidget: Text('Please choose one'),
                      initialValue: varietyResult,
                      onSaved: (value) {
                        if (value == null)
                          return null;
                        else {
